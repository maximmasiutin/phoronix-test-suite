#!/bin/sh
tar -xf qmcpack-4.0.0.tar.gz
cd qmcpack-4.0.0/build/
ADD_TO_CMAKE=""
...
#!/bin/sh
cd qmcpack-4.0.0/\$1
OMP_NUM_THREADS=1 mpirun --allow-run-as-root -np \$NUM_CPU_PHYSICAL_CORES ~/qmcpack-4.0.0/build/bin/qmcpack \$2 > \$LOG_FILE 2>&1
